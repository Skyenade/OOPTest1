...
public class PargolDemo {
	...
	public static void main(String[] args) {
		...
		PargolComputer[] inventory = new PargolComputer[maxComputers];
					...
					do {
						...
						if(numComputersToAdd > slotsAvailable) //not enough empty slots
							...
							for(int i = 0, addedCounter = 0; i < numComputersToAdd; i++) {
								if(inventory[i] == null) {
									PargolComputer c = new PargolComputer();
				...
				if(verifyPassword()) {
					...
					do {
						...
						if(editIndex == -1) //quit
							...
							do {
								PargolComputer c = inventory[editIndex];
								PargolComputer.displayComputer(c, editIndex);
								if(firstRun)
			...
			case 3: //display computers that match brandQuery
				...
				for(int i = 0; i < inventory.length; i++) {
					if(inventory[i] != null && brandQuery.toLowerCase().equals(inventory[i].getBrand().toLowerCase())) {
						PargolComputer.displayComputer(inventory[i], i);
						System.out.println();
						brandFound = true;
					}
				}
	...
	private static int slotsAvailable(PargolComputer[] inventory) {
		int slotsAvailable = 0;
		for(PargolComputer c : inventory) {
			if(c == null)
				slotsAvailable++;
		}
